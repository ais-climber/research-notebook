...
\begin{example*}
    Here's a classical example that illustrates how we can separate the expressive power of different classes using $\Theory{\Class}$.  Consider relational models $\Rel$ over the static language.  Here are some formulas that are valid in every relational model (see \cite{pacuit2017neighborhood} for details):
    \begin{itemize}
        \item $\Box_i(\varphi \land \psi) \to (\Box_i \varphi \land \Box_i \psi)$
        \item $(\Box_i \varphi \land \Box_i \psi) \to \Box_i(\varphi \land \psi)$
        \item $\Box_i \top$
    \end{itemize}
    This means that \emph{no} $\Model \in \Rel$ can satisfy their negations.  But plausibility models can: Let $\Model = \langle W, \leq, V \rangle$ be a plausibility model with $W = \set{a, b, c}$, $\leq$ is $\set{(a, b), (b, c)}$, and propositions $\set{p, q}$ with $V(p) = \set{a, c}, V(q) = \set{b, c}$. [A picture would be nice].  For this model, $\best(\semantics{p} \cap \semantics{q}) = \best(\set{c}) = \set{c}$, but $\best(\semantics{p}) \cap \best(\semantics{q}) = \set{a} \cap \set{b} = \emptyset$.  This means that for \emph{all} $w$, $\Model, w \not \Vdash \Box(\varphi \land \psi) \to (\Box \varphi \land \Box \psi)$ --- in other words, $\Model \models \neg (\Box(\varphi \land \psi) \to (\Box \varphi \land \Box \psi))$.  Consequently, this formula is in $\Theory{\Plaus}$, but \emph{not} in $\Theory{\Rel}$.

    Since we allow relational semantics in plausibility models, every $\Model \in \Rel$ is also in $\Plaus$.  So $\Plaus$ is strictly more expressive than $\Rel$: $\Theory{\Rel} \subset \Theory{\Plaus}$.
...
\[
    \Theory{\Rel} \subset \Theory{\Plaus} = \Theory{\NetModels} \subset \Theory{\Nbhd}
\]
These inclusions are mostly already-known.  $\Theory{\Rel} \subset \Theory{\Plaus}$ is a classical result, and I already gave the proof in the previous example.  In \cite{pacuit2017neighborhood}, Eric Pacuit shows that $\Theory{\Rel} \subset \Theory{\Nbhd}$ --- but we can show $\Theory{\Plaus} \subset \Theory{\Nbhd}$ by a straightforward extension of his argument.
...
The popular conception of neural networks is that they are ``black boxes'' whose behavior is difficult to understand.  This last fact tells us, however, that neural network inference (classification) corresponds precisely to defeasible conditionals.  When Thomas Icard last visited, he pointed out to me that this intuition is only helpful for describing the net's input/output behavior of a neural network, but not for explaining the net's hidden states.  Because of this, Thomas thinks that the ``neural network model'' formalism is on the wrong track (or at least, unenlightening).
